 ...
 * generate the `gpg.ssh.program` wrapper + allowed-signers file, build and
 * apply the `git config` entries, and persist the setup marker that stops
 * agents from re-running onboarding. The `ac2` command in `../cli/` is a
 * thin handler over this module.
 */
...
import { gitSignSocketPath, resolveAc2StateDir } from './bridge.js';
...
function resolveShimPath(): string {
  for (const candidate of ['./git.shim.js', './shim.js']) {
 ...
 * and bakes in the bridge socket path resolved *now* (git runs the wrapper
 * in the committing shell's environment, which need not match this one); an
 * `AC2_GIT_SIGN_SOCKET` already set in that environment still wins.
 */
export function writeGitSigningAssets(authorizedKey: string): {
  ...
  writeFileSync(
    ...
    [
      ...
      `: "\${AC2_GIT_SIGN_SOCKET:=${gitSignSocketPath()}}"`,
      'export AC2_GIT_SIGN_SOCKET',
