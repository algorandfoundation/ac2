...
import { decodeAddress, isValidAddress } from '@algorandfoundation/algokit-utils/common';
...
const NO_WALLET_KEY_MESSAGE =
  'No wallet key available: pair a wallet first (`openclaw ac2 pair`) so the ' +
  "controller's account key is known.";

/** Locate the standalone `git.shim.js` next to this module in the flat dist. */
function resolveShimPath(): string {
  for (const candidate of ['./git.shim.js', '../git/shim.js']) {
    const resolved = fileURLToPath(new URL(candidate, import.meta.url));
    if (existsSync(resolved)) return resolved;
  }
  return fileURLToPath(new URL('./git.shim.js', import.meta.url));
}

/**
 * Write the `gpg.ssh.program` wrapper + allowed-signers file under the AC2
 * state dir and return the paths. The wrapper pins the current Node binary.
 */
export function writeGitSigningAssets(authorizedKey: string): {
  wrapperPath: string;
  ...
  const wrapperPath = join(stateDir, 'ac2-ssh-sign');
  const shimPath = resolveShimPath();
  writeFileSync(
    wrapperPath,
    [
      '#!/bin/sh',
      '# Generated by `openclaw ac2 git-config` — routes git SSH signing to the AC2 wallet.',
  ...
  const allowedSignersPath = join(stateDir, 'allowed_signers');
  writeFileSync(allowedSignersPath, `* namespaces="git" ${authorizedKey}\n`);

  return { wrapperPath, allowedSignersPath };
}
...
/**
 * Build the `git config` key/value pairs for AC2 signing. When a PAT is
 ...
 */
export function buildGitConfigEntries(
  signingKeyLine: string,
  assets: { wrapperPath: string; allowedSignersPath: string },
  opts: GitConfigOptions,
): Array<[string, string]> {
  const entries: Array<[string, string]> = [
    ['gpg.format', 'ssh'],
    ['user.signingkey', `key::${signingKeyLine}`],
    ['gpg.ssh.program', assets.wrapperPath],
    ['gpg.ssh.allowedSignersFile', assets.allowedSignersPath],
    ['commit.gpgsign', 'true'],
  ];
