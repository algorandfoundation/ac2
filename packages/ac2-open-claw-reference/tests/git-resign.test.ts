...
describe('resignCommits', () => {
    ...
    it('passes git verify-commit when the key is an allowed signer', async () => {
        ...
        writeFileSync(
            signersPath,
            `ada@example.com ${toAuthorizedKeyLine(rawPublicKey)}\n`,
        );
    ...
    it('rejects with no_active_session when no wallet is paired', async () => {
        const repoDir = makeRepo();
        commit(repoDir, 'feat: offline');
        const result = await resignCommits({ repoDir }, {}, { manager: new SessionManager() });
